...
\textcolor{red}{Need to add: How to compile this .Rmd into pdf using command line}   
...
K-pop music (Korean popular music) is a genre of entertainment originated from South Korea \cite{lie2015k-pop}, and K-pop emphasizes on both dancing and singing components \cite{leung2012catching}.  

Therefore, K-pop shows are like an audio-visual treat, appealing to the general public \cite{pal2022k-pop}.  

Music: K-pop groups often showcase a wide range of musical styles within a single discography, and common styles include pop, hip-hop, rock, and R&B (Rhythm and Blues) \cite{liu2024newjeans, vogue2022best}.     

Lyrics:  
Upbeat and catchy - Many tracks are designed with high energy and memorable hooks, making them popular for dancing and enjoyable to listen to \cite{jo2023tracking, kim2022demystifying}.   

e.g. Debut single song - "Into the New World" (再次重逢的世界)^[<https://en.wikipedia.org/wiki/Into_the_New_World>]    

Performed by *Girls' Generation* (少女時代)^[<https://en.wikipedia.org/wiki/Girls%27_Generation>] in 2007.      

This song is considered a textbook example of K-pop music \cite{epstein2014into}.       

What do you think about the lyrics in this K-pop group?      
Positive, motivational, about women's empowerment \cite{tiensha2025between}   

Dance: complex choreography   

K-pop dance choreography is visually appealing not only on a live stage, but also on videos and social media \cite{chuyunoh2023dance, chuyunoh2022fandom}. The choreography is often complex where multiple dancers perform a group dance to the same music. The group dance features identical movements in most segments, while some parts of the solo performance are highlighted as a special video clip \cite{kim2025choreography}.    


Fashion: Dress, make-up, hair style, etc. [citation needed]      

Stage and video effects: Bright lights, video-editing for focus, etc. [citation needed]       

\textcolor{red}{Important: Write about why K-pop music is so popular across the globe.}   

K-pop has emerged popularity worldwide since the early 2010's \cite{khiun2013k-pop, sun2022identifying}. K-pop's international reputation stems from its versatile music, eye-catching performances, and the strategic use of social media, fostering a global community of dedicated fans who actively promote the artists and genre \cite{kim2022global, chen2023expansion}. Therefore, K-pop enjoys global fame -- not only across East Asian countries like China and Thailand \cite{malik2023korea-east-asia}, but also in the United States^[<https://bit.ly/4iIHpor>] and Western Europe^[<https://nolae.eu/blogs/news/diese-k-pop-konzerte-werden-2025-in-europa-stattfinden>] \cite{miroudot2024common}.      
...
Therefore, such TV programs often have high viewership \cite{ahn2022birth}. [more citations needed]   

For example, Mnet^[<https://en.wikipedia.org/wiki/Mnet_(TV_channel)>] is a television music channel famous for hosting idol-making survival shows, such as *Produce 101* and *Produce 48* \cite{capistrano2023into}. Their YouTube channel, *Mnet K-POP*,^[<http://www.youtube.com/@Mnet>] has more than 22 million subscribers worldwide.    

Thanks to translation, K-pop shows can be enjoyed by people outside the Korean-speaking community \cite{cruz2021cultural, yoon2023theorizing}.   

\textcolor{red}{Do we need to write something more?}  
...
e.g. Song Ha Young (宋河英) got 5.9 in dance and 9.8 in physical. Song Ha Young was a certified aerial yoga instructor before she first appeared in the *Idol School* reality show.^[<https://bit.ly/3ZJQipK>]   
...
It is reasonable to see a high correlation between vocal and dance scores, because the K-pop training usually includes vocal and dance lessons \cite{padget2017difficulties}. Theoretically, dance and physical should be highly correlated \cite{dance2024strength}, but in the Idol School dataset, we observed a slightly lower correlation in dance vs physical than in dance vs vocal. Physical strength is essential to dancing, but dance also includes other critical elements such as technique and aesthetic expression \cite{geukes2023mirror}.   

\textcolor{red}{Do more analysis to the Idol School data!}   

Plan: Use the `ggplot` graph of dance vs physical, and enlarge the picture.  

Figure \ref{fig:idol-conditional-drafts}:   
- Add separation lines for dance = 5.0 and physical = 3.0.   
- Add the correlation coefficients to each quadrant.   
- Increase the font of labels and axis ticks. They are too small compared with the figure size.   

```{r idol-conditional-drafts, fig.cap="Conditional scatter plot"}
# UNFINISHED HERE

cond_plot <- ggplot(data = idol_school, aes(x=Dance, y=Physical)) + 
  geom_point() + ggtitle("Dance vs Physical")

cond_plot <- scale_req(cond_plot) +
  geom_hline(yintercept = 3) + geom_vline(xintercept = 5)
...
Need to print and round the correlation coefficients!  
...
\textcolor{red}{Physical has median 3.25, so we use 3.0 as the separation threshold.}    

Only in the physical < 3 group has a mid-level correlation with the dance scores.  

However, when the physical performance is already below 3 points, ...  

\textcolor{red}{How to interpret the physical-lower group's high correlation with dance scores?}   

Consider: Create another scatter plot of this group.   

Need to print and round the correlation coefficients!   

```{r idol-dance-cond-on-physical-drafts}
# UNFINISHED HERE
physical_upper = idol_school[which(idol_school$Physical >= 3),]
physical_lower = idol_school[which(idol_school$Physical < 3),]

# Correlation coefficients
# cor(physical_upper$Dance, physical_upper$Physical)  # 0.2577632
# cor(physical_lower$Dance, physical_lower$Physical) # 0.5648321

# Standard deviation
# sd(physical_upper$Dance) # 2.000308
# sd(physical_lower$Dance) # 1.891095
```

## *Idol School*: Additional Resources

For the contestants eliminated from the show, some continued to try out in other K-pop reality shows to earn a new position in debut, while some others left the K-pop industry to pursue other goals.^[<https://www.ptt.cc/bbs/fromis_9/M.1555819461.A.C73.html>]   
...
```{r idol-school-fabricated-drafts}
# UNFINISHED HERE
...
Due to the manipulation of votes, it is meaningless to predict the final rankings in the *Idol School* survival show. That's why we decided to analyze only the contestant scores given by the professional instructors. These scores are a relatively good benchmark of raw performance, especially that they were not artificially changed to favor any particular contestant.    
...
Let's examine how Korean and Japanese participants scored in the first evaluation. We noticed that the majority of contestants from Japan were placed in D or F (lowest grades), while very few contestants from Korea did. Although the Korean K-pop coaches criticized the Japanese contestants for their performance, we would like to emphasize that the Idol training process differs greatly in Korea and Japan.^[<https://www.adaymag.com/2020/10/22/japanese-vs-korean-idol.html>] In Korea, the primary focus is on vocal and dance techniques \cite{sim2017hybridity}, while in Japan, the Idol training values artistic interpretation and individual expressiveness \cite{alice-jiye-lee-2023}.    

In the show, many Japanese contestants bursted into tears due to the harsh feedback given by the Korean K-pop instructors.^[<https://star.ettoday.net/news/1192170>] The Japanese contestants enjoyed highly positive responses in their own country, so they thought they would receive an A in the evaluation during *Produce 48*. Instead, most of them received an F (failure). After the first evaluation, some of the Japanese contestants withdrew from the show for various reasons. Two of them did not even complete the second evaluation.^[<https://star.ettoday.net/news/1181576>]     
...
The remaining three did not do so well in the audience votes. The three contestants are Na Go Eun (羅高恩), Lee Ha Eun (李河恩), and Hwang So Yeon (黃召硯). They still deserve an honorable mention here.   
...
\textcolor{red}{Description here}    

Yabuki Nako (矢吹奈子): F $\rightarrow$ A    

\textcolor{red}{Description here}    

Matsui Jurina (松井珠理奈): B $\rightarrow$ B    
