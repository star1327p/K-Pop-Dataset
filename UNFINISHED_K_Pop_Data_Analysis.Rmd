...
If the readers prefer a graphical summary of the data distributions, Figure \ref{fig:ggplot2-drafts} is the **box plot** of the vocal, dance, and physical scores in the *Idol School* show.    

We visualize the data using `ggplot2` \cite{ggplot2} to compare the three sets of scores.      

\textcolor{red}{What is the purpose of a box plot?}   

Central bold line = median  
Box = from 1st quartile to 3rd quartile   
Whiskers = Minimum and maximum values   

<https://medium.com/analytics-vidhya/the-box-plot-a-simple-but-informative-visualization-cacc20d9ff25>

\textcolor{red}{Explain the \texttt{tidyr} and long-data parts}  

We are using the `R` package `tidyr` \cite{tidyr} to convert wide data into long format.   
...
```{r ggplot2-drafts, fig.cap="Box plot of the vocal, dance, and physical scores in the \\textit{Idol School} show", echo=FALSE}
# UNFINISHED HERE
...
On the other hand, Lee Hae In (李海印) received a score of 1 in vocal, because she unfortunately lost her voice and could barely sing at that time.  

The median score of vocal is 4.95 (rounding to 5), which indicates decent performance.   
...
\textcolor{red}{Use \texttt{ggplot}!}    

Explain the `gridExtra` package \cite{gridExtra}.   

Figure \ref{fig:idol-scores-scatter-plot-drafts} contains the three scatterplots.   

\textcolor{red}{Also need to explain the code}   

\textcolor{red}{Alternative: Hide the code and only show the graph itself}   

```{r idol-scores-scatter-plot-drafts, fig.height=2, fig.width=6.5, fig.cap="Scatterplots to show the pairwise correlation between the vocal, dance, and physical scores. The graphs from left to right are: Vocal vs Dance, Vocal vs Physical, and Dance vs Physical.", echo=FALSE}
